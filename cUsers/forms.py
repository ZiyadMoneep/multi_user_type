...
from .models import Recruiter, Applicant, User, Secretary, Supervisor
...
class SecretaryCustomUserCreationForm(UserCreationForm):
    first_name = forms.CharField(
        max_length=100,
        required=True,
        help_text="Enter Your first name",
    ...
    def save(self):
        ...
        user.age = self.cleaned_data.get('age')
        user.save()
        secretary = Secretary.objects.create(user=user)
        secretary.phone_number = self.cleaned_data.get('phone_number')
        secretary.save()
        return user


class SupervisorCustomUserCreationForm(UserCreationForm):
    first_name = forms.CharField(
        max_length=100,
        required=True,
        help_text="Enter Your first name",
